/* ===== DATA ===== */
const products = [
  { id:1, name:"Midnight Blue", price:349000, tag:"Woody Spicy", badge:"Best Seller", rating:4.9, reviews:128,
    desc:"Aroma maskulin yang tajam dan segar, cocok untuk penggunaan malam hari atau acara formal.",
    notes:{top:"Bergamot",mid:"Sichuan Pepper",base:"Ambroxan"}, longevity:"8-10 Jam",
    img:"https://images.unsplash.com/photo-1541643600914-78b084683601?w=500" },
  { id:2, name:"Rose Velvet", price:420000, tag:"Floral Sweet", badge:"New Arrival", rating:4.8, reviews:84,
    desc:"Sentuhan mawar yang lembut dipadu dengan vanilla hangat. Elegan dan sangat feminin.",
    notes:{top:"Damask Rose",mid:"Vanilla",base:"White Musk"}, longevity:"6-8 Jam",
    img:"https://images.unsplash.com/photo-1585120040315-2241b774ad0f?w=500" },
  { id:3, name:"Oud Wood", price:550000, tag:"Oriental Woody", badge:"Best Seller", rating:5.0, reviews:210,
    desc:"Kehangatan kayu gaharu langka yang eksotis. Pilihan untuk karakter yang kuat dan misterius.",
    notes:{top:"Cardamom",mid:"Agarwood (Oud)",base:"Sandalwood"}, longevity:"12+ Jam",
    img:"https://images.unsplash.com/photo-1594035910387-fea47794261f?w=500" },
  { id:4, name:"Ocean Mist", price:299000, tag:"Aquatic Fresh", badge:"", rating:4.7, reviews:67,
    desc:"Sensasi kesegaran laut yang menenangkan. Sangat cocok untuk aktivitas outdoor di siang hari.",
    notes:{top:"Sea Salt",mid:"Sage",base:"Grapefruit"}, longevity:"5-7 Jam",
    img:"https://images.unsplash.com/photo-1592945403244-b3fbafd7f539?w=500" },
  { id:5, name:"Amber Noir", price:480000, tag:"Oriental Woody", badge:"New Arrival", rating:4.9, reviews:43,
    desc:"Perpaduan amber oriental dan musc hitam yang memukau. Sensual dan mewah.",
    notes:{top:"Black Pepper",mid:"Amber",base:"Dark Musk"}, longevity:"10-12 Jam",
    img:"https://images.unsplash.com/photo-1523293182086-7651a899d37f?w=500" },
  { id:6, name:"Cherry Blossom", price:310000, tag:"Floral Sweet", badge:"", rating:4.6, reviews:55,
    desc:"Aroma bunga sakura yang lembut dan menyegarkan. Cocok untuk penggunaan sehari-hari.",
    notes:{top:"Lychee",mid:"Cherry Blossom",base:"Soft Musk"}, longevity:"5-7 Jam",
    img:"https://images.unsplash.com/photo-1588776814546-1ffeddb3f1a8?w=500" },
];

const testimonials = [
  { name:"Rian Adrianto", text:"Wanginya tahan lama banget, Midnight Blue beneran bikin PD seharian!", stars:5 },
  { name:"Sarah Clarissa", text:"Rose Velvet lembut banget aromanya. Gak bikin pusing. Packagingnya mewah.", stars:5 },
  { name:"Budi Santoso", text:"Oud Wood pilihan terbaik buat acara formal. Banyak yang nanya parfum apa haha.", stars:5 },
  { name:"Dinda Ayu", text:"Ocean Mist cocok banget buat aku yang aktif. Wanginya fresh dan clean!", stars:4 },
];
...
function getUsers() { return LS.get('decane_users') || {}; }
function saveUsers(u) { LS.set('decane_users', u); }
...
function doRegister() {
  ...
  if (!name || !email || !pass) return showErr('regError', 'Semua field wajib diisi!');
  if (!email.includes('@')) return showErr('regError', 'Format email tidak valid!');
  if (pass.length < 6) return showErr('regError', 'Password minimal 6 karakter!');
  const users = getUsers();
  if (users[email]) return showErr('regError', 'Email sudah terdaftar!');
  users[email] = { name, email, phone, pass, createdAt: new Date().toISOString() };
  saveUsers(users);
...
function doLogin() {
  ...
  const users = getUsers();
  if (!users[email] || users[email].pass !== pass) return showErr('loginError', 'Email atau password salah!');
  currentUser = users[email];
  enterApp();
}

function doGuest() {
  currentUser = { name: 'Tamu', email: 'guest@decane.id', phone: '', isGuest: true };
  enterApp();
}
...
function renderProducts(list) {
  ...
  if (!grid) return;
  if (!list.length) { grid.innerHTML = '<p style="color:var(--gray);font-size:13px;grid-column:span 2;text-align:center;padding:30px 0;">Produk tidak ditemukan</p>'; return; }
  grid.innerHTML = list.map(p => {
    ...
    return `<div class="card">
      <div class="card-img-wrap" onclick="viewDetail(${p.id})">
        ...
        <button class="wishlist-btn ${inWish ? 'active' : ''}" onclick="event.stopPropagation(); toggleWish(${p.id}, this)">
...
function renderTestimonials() {
  ...
  if (!area) return;
  area.innerHTML = testimonials.map(t => {
...
function filterChip(el, val) {
  ...
  if (val !== 'all') {
    if (val === 'Best Seller' || val === 'New Arrival') filtered = products.filter(p => p.badge === val);
...
function toggleWish(id, btn) {
  ...
  if (idx === -1) { wishlist.push(id); btn.classList.add('active'); btn.innerHTML = '<i class="fas fa-heart"></i>'; showToast('Ditambahkan ke Wishlist ❤️'); }
  else { wishlist.splice(idx, 1); btn.classList.remove('active'); btn.innerHTML = '<i class="far fa-heart"></i>'; showToast('Dihapus dari Wishlist'); }
  saveUserData();
}

function toggleWishModal(id, btn) {
  const idx = wishlist.indexOf(id);
  if (idx === -1) { wishlist.push(id); btn.style.color = 'var(--danger)'; btn.innerHTML = '<i class="fas fa-heart"></i>'; showToast('Ditambahkan ke Wishlist ❤️'); }
  else { wishlist.splice(idx, 1); btn.style.color = '#ccc'; btn.innerHTML = '<i class="far fa-heart"></i>'; showToast('Dihapus dari Wishlist'); }
  saveUserData();
  renderProducts(products);
}
...
function applyPromo() {
  ...
  if (code === 'DECANE10') showToast('🎉 Promo berhasil! Ongkir gratis diterapkan');
...
function sendToWA(total) {
  ...
  // Save order
  orders.push({ id: Date.now(), items: Object.entries(cart).map(([id,qty])=>({id:parseInt(id),qty})), total, name, addr, status:'Pending', date: new Date().toLocaleDateString('id-ID') });
  saveUserData();

  const admin = adminList[adminIdx % adminList.length];
  adminIdx++;
  window.open(`https://wa.me/${admin}?text=${encodeURIComponent(msg)}`, '_blank');
  cart = {}; updateCartBadge(); saveUserData();
  closeModal(); switchPage('home');
...
function updateProfile() {
  ...
  document.getElementById('profileEmail').textContent = currentUser.isGuest ? 'Mode Tamu' : currentUser.email;
}
...
document.getElementById('appModal').onclick = e => { if (e.target.id === 'appModal') closeModal(); };

function renderOrderHistory(content) {
  const orderHtml = orders.length ? orders.slice().reverse().map(o => {
    const names = o.items.map(i => products.find(p => p.id == i.id)?.name).filter(Boolean).join(', ');
    return `<div class="order-card">
      ...
      <div style="font-weight:700;color:var(--dark);">Rp ${o.total.toLocaleString('id-ID')}</div>
    </div>`;
  }).join('') : '<div class="wishlist-empty"><i class="fas fa-box" style="font-size:40px;color:#eee;display:block;margin-bottom:10px;"></i><p>Belum ada pesanan</p></div>';
